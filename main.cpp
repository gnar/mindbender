...
int Main_::run(int argc, char** argv)
{
	...
	cout << "MindBender comes with ABSOLUTELY NO WARRANTY. See GPL.txt (included in this distribution) for details." << endl;
	cout << endl;
	...
	for (;;)
	{
		...
		switch (cmd)
		{
			case 'c': cmd_game_file = optarg; break;
			case 'i': cmd_show_info = true; break;
			case 'x': cmd_disable_extensions = true; break;
			case 'y': cmd_disable_gc = true; break;
			case 't': cmd_max_tex_size = std::atoi(optarg); break;
			case ':':
			case 'h':
			case '?': cmd_show_cmdline = true; break;
		}
	}
	...
	try {
		...
		SDL_WM_SetCaption(GetConfiguration().GetTitle().c_str(), 0);
	...
	//	DoneSound();
		...
		std::cerr << err.what() << std::endl; error = true;
	} catch (std::exception e) {
		std::cerr << e.what() << std::endl; error = true;
	}
