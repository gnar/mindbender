...
void EventHandler::Save(CLSerialSaver &S) {
    ...
    auto argc = (unsigned int) (args.size());
    S.IO(argc);

    for (auto &arg : args) {
        auto wildcard = int(arg.wildcard);
        S.IO(wildcard);

        if (!wildcard) CLValue::save(S, arg.value);
    }
}
...
void EventHandler::Load(CLSerialLoader &S) {
    ...
    for (auto &arg : args) {
        int wildcard;
        S.IO(wildcard);
        arg.wildcard = bool(wildcard);

        if (wildcard)
            arg.value = CLValue::Null();
        else
            arg.value = CLValue::load(S);
    }
}
...
void EventManager::Update(float dt) // each frame
{
    ...
    auto ec_it = classes.begin(), ec_end = classes.end();
...
void EventManager::AddEventClassEntry(const std::string &event_class) {
    auto it = classes.find(event_class);
...
CLValue EventManager::Signal(CLContext *context, const std::string &event_name, std::vector<CLValue> &values) {
    auto it = handlers.begin(), end = handlers.end();
    EventHandler *best_match = nullptr;
    ...
    while (it != end) {
        ...
        if (score > best_score) {
            ...
            auto ec_it = classes.find(handler.GetClass());
    ...
    if (best_match == nullptr) return CLValue();

    // Kill old event thread if any.
    auto ec_it = classes.find(best_match->GetClass());
...
void EventManager::Save(CLSerialSaver &S) {
    ...
    for (auto &classe : classes) {
        std::string class_id = classe.first;
        EventClass &EC = classe.second;
