...
void Camera::Update(float dt) {
    ...
    if (!dest_room.isNull()) {
        if (dest_x >= GET_ROOM(dest_room)->GetWidth() - window.width) dest_x = GET_ROOM(dest_room)->GetWidth() -
                                                                               window.width;
        if (dest_y >= GET_ROOM(dest_room)->GetHeight() - window.height) dest_y = GET_ROOM(dest_room)->GetHeight() -
                                                                                 window.height;
    } else {
        dest_x = dest_y = 0;
    }
    ...
    switch (scroll_mode) {
        ...
        case SCROLL:
            if (cur_room.op_eq(dest_room).getBoolUnsave()) // on room change, snap anyway
            {
                ...
                float steplen = (dt * float(scroll_speed)) /
                                1000.0f; // maximum movement in this frame in pixels without acceleration
                if (scroll_accel != 0) {
                    float dist = std::sqrt((cur_x - dest_x) * (cur_x - dest_x) +
                                           (cur_y - dest_y) * (cur_y - dest_y)); // src->dest distance / 1000
...
void Camera::Load(CLSerialLoader &S) {
    ...
    // camera modes
    S.IO(tmp);
    camera_mode = static_cast<CameraMode>(tmp);
    S.IO(tmp);
    scroll_mode = static_cast<ScrollMode>(tmp);
