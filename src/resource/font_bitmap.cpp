...
namespace Res {
    ...
    void Font_Bitmap::LoadXML() {
        ...
        glyphs.resize(256, (Glyph *) nullptr);

        TiXmlElement *font_elem = doc->FirstChildElement("font");
        TiXmlElement *glyphs_elem = font_elem ? font_elem->FirstChildElement("glyphs") : nullptr;
        while (glyphs_elem) {
    ...
    void Font_Bitmap::LoadGlyphElement(TiXmlElement *glyph_elem) {
        const char *image_attr = glyph_elem->Attribute("image");
        assert(image_attr);
