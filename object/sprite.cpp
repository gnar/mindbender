...
void Sprite::Update() {
    ...
    auto delta_time = float(new_time - last_time);
    last_time = new_time;
...
bool Sprite::get(CLValue &key, CLValue &val) {
    if (key.type == CL_STRING) {
        ...
        if (k == "Draw") {
            val = this->method_draw;
            return true;
        } else if (k == "Rewind") {
            val = this->method_rewind;
            return true;
        } else if (k == "IsFinished") {
            val = this->method_is_finished;
            return true;
        } else if (k == "Translate") {
            val = this->method_translate;
            return true;
        } else if (k == "track") {
            val = CLValue(new CLString(getContext(), sprite_state->GetTrack()));
            return true;
        } else if (k == "x") {
            val = CLValue(this->x);
            return true;
            ...
            val = CLValue(static_cast<int>(scale * 100.0f));
            return true;
        }
    }
...
void Sprite::Save(CLSerialSaver &S, Sprite *sprite) {
    ...
    auto delta_time = static_cast<int>(now - sprite->last_time);
    S.IO(delta_time);
...
Sprite *Sprite::Load(CLSerialLoader &S) {
    ...
    auto *sprite = new Sprite(S.getContext(), res_id);
    S.addPtr(sprite);
    ...
    switch (lt) {
        case 0:
            sprite->last_tick = Game().GetFrameCount() - 100;
            break;
        case 1:
            sprite->last_tick = Game().GetFrameCount();
            break;
        case 2:
            sprite->last_tick = Game().GetFrameCount() - 1;
            break;
        default:
            assert(0);
    }
