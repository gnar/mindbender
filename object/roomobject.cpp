...
void RoomObject::set(CLValue &key, CLValue &val)
{
	if (key.type == CL_STRING)
	{
		...
		// properties
		if (k == "x") {
			this->x = val.toInt(); return;
		} else if (k == "y") {
			this->y = val.toInt(); return;
		} else if (k == "layer") {
			SetLayer(val.toInt()); return;
		} else if (k == "hide") {
			Hide(val.toBool()); return;
		} else if (k == "name") {
			this->name = GET_STRING(val)->get(); return;
		} else if (k == "room") { //ro
			clog << "<actor/item>.room is read-only!" << endl; return;
		} else if (k == "hotspot") {
			this->hotspot = val; return;
		} else if (k == "bag") { //ro
			clog << "<actor/item>.bag is read-only!" << endl; return;

		// methods, all ro
		} else if (k == "AddBagItem") { //ro
			clog << "<actor/item>.AddBagItem is read-only!" << endl; return;
		} else if (k == "RemBagItem") { //ro
			clog << "<actor/item>.RemBagItem is read-only!" << endl; return;
		} else if (k == "HasBagItem") { //ro
			clog << "<actor/item>.HasBagItem is read-only!" << endl; return;
		}
	}
...
bool RoomObject::get(CLValue &key, CLValue &val) // returns true if key existed
{
	if (key.type == CL_STRING)
	{
		...
		if (k == "x") {
			...
			val = CLValue(static_cast<int>(GetLayer())); return true;
		} else if (k == "hide") {
			val = IsHidden() ? CLValue::True() : CLValue::False(); return true;
		} else if (k == "name") {
			val = CLValue(new CLString(getContext(), name.c_str())); return true;
		} else if (k == "room") {
			val = this->room; return true;
		} else if (k == "hotspot") {
			val = this->hotspot; return true;
		} else if (k == "bag") {
			val = this->cached_inventory; return true;
	
		// methods
		} else if (k == "AddBagItem") {
			val = method_add_bagitem; return true;
		} else if (k == "RemBagItem") {
			val = method_rem_bagitem; return true;
		} else if (k == "HasBagItem") {
			val = method_has_bagitem; return true;
		}
	}
...
void RoomObject::Load(CLSerialLoader &S, RoomObject *obj)
{
	...
	S.IO(obj->x);
	S.IO(obj->y);
	S.IO(utmp); obj->layer = static_cast<size_t>(utmp);
	S.IO(utmp); obj->hidden = utmp;
