...
public:
	...
	void SetSize(int width, int height, int p = 1) { planes[p].width = width; planes[p].height = height; }
	...
	void SetConstantScaling(float s) { scale_mode = SCALE_CONSTANT; scale_constant = s; }
	void SetPerspectiveScaling(int horizon, int base) 
		{ scale_mode = SCALE_PERSPECTIVE; scale_y_horizon = horizon; scale_y_base = base; }
