...
void Actor::Stop() // stop all actions
{
	...
	{
		case STILL: break; //ok
		case WALKING: Walk_Stop(); break;
		case TALKING: Talk_Stop(); break;
		case SPECIAL: Special_Stop(); break;
	}
...
void Actor::Talk_Parse(const std::string &text) 
{
	...
	for (size_t i=0; i<text.size(); ++i)
	{
		...
		if (ch != '<')
		{
			...
			std::string token = "";
			++i; if (i >= text.size()) goto done; ch = cstr[i];
			while (ch != '>')
			{
				token += ch;
				++i; if (i >= text.size()) goto done; ch = cstr[i];
...
bool Actor::WalkTo(int to_x, int to_y)
{
	...
	{	// Override room path?
		Shape *path2 = !path_override.isNull() ? GET_SHAPE(path_override) : 0;
		if (path2) path = path2;
	}
	...
	{ // HACKS
		...
		if (!path->Hit(x, y)) 
		{
			...
			else if (path->Hit(x-1, y-1)) { x-=1; y-=1; }
			else if (path->Hit(x+1, y-1)) { x+=1; y-=1; }
			else if (path->Hit(x-1, y+1)) { x-=1; y+=1; }
			else if (path->Hit(x+1, y+1)) { x+=1; y+=1; }
	...
	Walk_dest_x = to_x;
	Walk_dest_y = to_y;
	Walk_segm1 = *(Walk_route.begin()); Walk_route.pop_front();
	Walk_segm2 = *(Walk_route.begin()); Walk_route.pop_front();
...
void Actor::Walk_Update(float dt)
{
	...
	if (Walk_segm1.x == Walk_segm2.x) // vertical walk?
	{
		...
		progress = raw_progress * std::sqrt((1 + yx_speed_2 * yx_ratio_2) / (1 + yx_speed_2)); // scaled down due to room->yx_ratio
	}
...
void Actor::set(CLValue &key, CLValue &val)
{
	if (key.type == CL_STRING)
	{
		...
		if (k == "WalkTo") {
			clog << "<actor>.WalkTo is read-only!" << endl; return;
		} else if (k == "IsWalking") {
			clog << "<actor>.IsWalking is read-only!" << endl; return;
		} else if (k == "IsTalking") {
			clog << "<actor>.IsTalking is read-only!" << endl; return;
		} else if (k == "Talk") {
			clog << "<actor>.Talk is read-only!" << endl; return;
		} else if (k == "TalkAppend") {
			clog << "<actor>.TalkAppend is read-only!" << endl; return;
		} else if (k == "Stop") {
			clog << "<actor>.Stop is read-only!" << endl; return;
		} else if (k == "Perform") {
			clog << "<actor>.Perform is read-only!" << endl; return;

		// properties
		} else if (k == "x") {
			SetPositionX(val.toInt()); Rescale(); return;
		} else if (k == "y") {
			SetPositionY(val.toInt()); Rescale(); return;
		} else if (k == "width") {
			this->width = val.toInt(); return;
		} else if (k == "height") {
			this->height = val.toInt(); return;
		} else if (k == "scale") {
			this->scale = float(val.toInt()) / 100; return;
		} else if (k == "font") {
			this->font = val; return;
		} else if (k == "speed") {
			this->speed = val.toInt(); return;
		} else if (k == "dir") {
			this->dir = val.toInt(); return;
		} else if (k == "walk_x") { // ro
			clog << "<actor>.walk_x is read-only!" << endl; return;
		} else if (k == "walk_y") { // ro
			clog << "<actor>.walk_y is read-only!" << endl; return;

		} else if (k == "still_sprite") {
			this->sprite_still = val; return;
		} else if (k == "walk_sprite") {
			this->sprite_walking = val; return;
		} else if (k == "talk_sprite") {
			this->sprite_talking = val; return;

		} else if (k == "path_override") {
			this->path_override = val; return;
		} else if (k == "no_auto_scale") {
			this->no_auto_scale = val.toBool(); return;
...
bool Actor::get(CLValue &key, CLValue &val) // returns true if key existed
{
	if (key.type == CL_STRING)
	{
		...
		// methods
		if (k == "WalkTo") {
			val = method_walkto; return true;
		} else if (k == "IsWalking") {
			val = method_iswalking; return true;
		} else if (k == "IsTalking") {
			val = method_istalking; return true;
		} else if (k == "Talk") {
			val = method_talk; return true;
		} else if (k == "TalkAppend") {
			val = method_append_talk; return true;
		} else if (k == "Stop") {
			val = method_stop; return true;
		} else if (k == "Perform") {
			val = method_perform; return true;

		// properties
		} else if (k == "x") {
			val = CLValue(GetPositionX()); return true;
		} else if (k == "y") {
			val = CLValue(GetPositionY()); return true;
		} else if (k == "width") {
			val = CLValue(width); return true;
		} else if (k == "height") {
			val = CLValue(height); return true;
		} else if (k == "scale") {
			val = CLValue(int(scale * 100.0f)); return true;
		} else if (k == "font") {
			val = font; return true;
		} else if (k == "speed") {
			val = CLValue(speed); return true;
		} else if (k == "dir") {
			val = CLValue(this->dir); return true;

		} else if (k == "walk_x") { // ro
			val = CLValue(Walk_dest_x); return true;
		} else if (k == "walk_y") { // ro
			val = CLValue(Walk_dest_y); return true;

		} else if (k == "still_sprite") {
			val = this->sprite_still; return true;
		} else if (k == "walk_sprite") {
			val = this->sprite_walking; return true;
		} else if (k == "talk_sprite") {
			val = this->sprite_talking; return true;

		} else if (k == "path_override") {
			val = this->path_override; return true;
		} else if (k == "no_auto_scale") { 
			val = this->no_auto_scale ? CLValue::True() : CLValue::False(); return true;
...
void Actor::Save(CLSerialSaver &S, Actor *actor)
{
	...
	
	{
		...
		std::list<Vertex>::iterator it = actor->Walk_route.begin(), end = actor->Walk_route.end();
		for (;it!=end;++it)
		{
			Vertex &V = *it;
			S.IO(V.x);
...
Actor *Actor::Load(CLSerialLoader &S)
{
	...
	Actor *actor = new Actor(S.getContext()); S.addPtr(actor);
...

	{
		...
		S.IO(tmp); size_t Talk_queue_size = static_cast<int>(tmp);
		for (size_t i=0; i<Talk_queue_size; ++i)
		{
			actor->Talk_queue.push(TalkItem());
			TalkItem &TI = actor->Talk_queue.front();
			
			S.IO(tmp); TI.type = static_cast<TalkItem::Type>(tmp);
			S.IO(TI.text);	
			S.IO(TI.delay);
		}
	}
	
	{
		...
		//std::list<Vertex>  Walk_route
		S.IO(tmp); size_t Walk_route_size = static_cast<size_t>(tmp);
