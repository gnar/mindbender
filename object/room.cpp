...
bool Room::get(CLValue &key, CLValue &val) {
    if (key.type == CL_STRING) {
        ...
        if (k == "width") {
            val = CLValue(this->width);
            return true;
        } else if (k == "height") {
            val = CLValue(this->height);
            return true;
        } else if (k == "tilt") {
            val = CLValue(this->yx_ratio);
            return true;
        } else if (k == "background") {
            val = this->planes[1].background;
            return true; //TODO
        } else if (k == "path") {
            val = this->path;
            return true;
            ...
            val = this->method_objectat;
            return true;
        } else if (k == "AddObject") {
            val = this->method_addobject;
            return true;
        } else if (k == "RemObject") {
            val = this->method_remobject;
            return true;
        } else if (k == "SetConstantScaling") {
            val = this->method_set_constant_scaling;
            return true;
        } else if (k == "SetPerspectiveScaling") {
            val = this->method_set_perspective_scaling;
            return true;
        } else if (k == "EnableScaling") {
            val = this->method_enable_scaling;
            return true;
        } else if (k == "DisableScaling") {
            val = this->method_disable_scaling;
            return true;
        }
    }
...
void Room::set(CLValue &key, CLValue &val) {
    if (key.type == CL_STRING) {
        ...
        if (k == "width") {
            ...
            this->yx_ratio = val.toInt();
            return;
        } else if (k == "background") {
            this->planes[1].background = val;
            return; //TODO
        } else if (k == "path") {
            this->path = val;
            return;
            ...
            clog << "xxxxxxxxxx member is read-only. " << endl;
            return;
        }
    }
...
void Room::markReferenced() // GC
{
    ...
    for (int p = 0; p < 3; ++p) {
        ...
        for (size_t i = 0; i < NUM_LAYERS; ++i) {
            auto it = planes[p].objects[i].begin(), end = planes[p].objects[i].end();
