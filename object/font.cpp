...
bool FontSet::get(CLValue &key, CLValue &val)
{
	if (key.type == CL_STRING)
	{
		...
		if (k == "x") {
			val = CLValue(x); return true;
		} else if (k == "y") {
			val = CLValue(y); return true;

		} else if (k == "r") {
			val = CLValue(r); return true;
		} else if (k == "g") {
			val = CLValue(g); return true;
		} else if (k == "b") {
			val = CLValue(b); return true;
		} else if (k == "a") {
			val = CLValue(a); return true;

		} else if (k == "Print") {
			val = method_print; return true;
		} else if (k == "PrintCenter") {
			val = method_print_center; return true;
		} else if (k == "GetWidth") {
			val = method_get_length; return true;
		} else if (k == "GetHeight") {
			val = method_get_height; return true;
		}
	}
...
FontSet *FontSet::Load(CLSerialLoader &S)
{
	...
	FontSet *font = new FontSet(S.getContext(), res_id); S.addPtr(font);
