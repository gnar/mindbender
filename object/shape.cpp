...
RectangularShape::~RectangularShape() = default;
...
RectangularShape *RectangularShape::Load(CLSerialLoader &S) {
    ...
    auto *shape = new RectangularShape(S.getContext(), pos_x, pos_y, width, height);
    S.addPtr(shape);
...
CircularShape::~CircularShape() = default;
...
CircularShape *CircularShape::Load(CLSerialLoader &S) {
    ...
    auto *shape = new CircularShape(S.getContext(), pos_x, pos_y, radius);
    S.addPtr(shape);
    TableObject::Load(S, shape); // load tableobject data
    return shape;
}
...
PolygonShape *PolygonShape::Load(CLSerialLoader &S) {
    ...
    auto *shape = new PolygonShape(S.getContext(), res_id);
    S.addPtr(shape);
    TableObject::Load(S, shape); // load tableobject data
    return shape;
}
