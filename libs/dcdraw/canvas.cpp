...
namespace DCDraw
{
	...
	Canvas::Canvas(Format fmt, int width, int height)
	{
		...
		switch (fmt)
		{
			case ARGB4444:
			case RGBA4444:
			case RGB565: pitch = width * 2; break;

			case RGB:    pitch = width * 3; break;

			case RGBA: 
			case ARGB:   pitch = width * 4; break;
		}
	...
	Canvas Canvas::Clone()
	{
		...
		if (bitmap.get())
		{
			clone.bitmap.reset(bitmap.get()->Clone());
		}
		return clone;
	} 
	...
	void Canvas::GetPixel(int x, int y, unsigned char &r, unsigned char &g, unsigned char &b, unsigned char &a)
	{
		...
		{
			case RGBA4444: {
				unsigned short *pix = (unsigned short*)&row[x*2]; //FIXME: typedef a 16 bit type
			...
			case RGB:  r = row[x*3+0]; g = row[x*3+1]; b = row[x*3+2]; a = 255; break;
			case RGBA: r = row[x*4+0]; g = row[x*4+1]; b = row[x*4+2]; a = row[x*4+3]; break;
			case ARGB: a = row[x*4+0]; r = row[x*4+1]; g = row[x*4+2]; b = row[x*4+3]; break;
		}
	}
	...
	void Canvas::SetPixel(int x, int y, unsigned char r, unsigned char g, unsigned char b, unsigned char a)
	{
		...
		switch (bitmap->fmt)
		{
			...
			case RGB:  row[x*3+0] = r; row[x*3+1] = g; row[x*3+2] = b; break;
			case RGBA: row[x*4+0] = r; row[x*4+1] = g; row[x*4+2] = b; row[x*4+3] = a; break;
			case ARGB: row[x*4+0] = a; row[x*4+1] = r; row[x*4+2] = g; row[x*4+3] = b; break;
		}
	}
