...
SushiModule::SushiModule() : CLModule("adv") {
    ...
    registerFunction("Random", "adv_random", &random_);
...
SushiModule::~SushiModule() = default;
...
static DECL_FUNC(random_) // adv.Random(max), adv.Random(min, max), adv.Random()
{
    switch (args.size()) {
        case 0:
            return CLValue(std::rand());
...
static DECL_FUNC(camera_getoffset) {
    ...
    GET_TABLE(retv)->set(key = CLValue(new CLString(thread.getContext(), "x")),
                         val = CLValue(Game().GetCamera().GetOffsetX()));
    GET_TABLE(retv)->set(key = CLValue(new CLString(thread.getContext(), "y")),
                         val = CLValue(Game().GetCamera().GetOffsetY()));
    return retv;
}
