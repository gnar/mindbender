...
public:
    void save(CLSerialSaver &S, CLUserData *userdata) override {
        int tmp;
    ...
    CLUserData *load(CLSerialLoader &S) override {
        int tmp;
        S.IO(tmp);

        switch (tmp) {
            case ID_SHAPE_RECT:
                return RectangularShape::Load(S);
            case ID_SHAPE_CIRCLE:
                return CircularShape::Load(S);
            case ID_SHAPE_POLYGON:
                return PolygonShape::Load(S);
            case ID_ROOM:
                return Room::Load(S);
            case ID_ITEM:
                return Item::Load(S);
            case ID_ACTOR:
                return Actor::Load(S);
            case ID_SPRITE:
                return Sprite::Load(S);
            case ID_BAGITEM:
                return BagItem::Load(S);
            case ID_SOUND:
                return Sound::Load(S);
            case ID_FONT:
                return FontSet::Load(S);
            case ID_TIMER:
                return Timer::Load(S);
            default:
                assert(false);
