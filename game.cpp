...
void Game_::OnMouseDown(int x, int y, int btn)
{
	...
	std::vector<CLValue> args;
	args.push_back(CLValue(x));
	args.push_back(CLValue(y));

	// check if mouse is over a room (camera) 
	int room_x = x, room_y = y;
	if (camera.ScreenToRoomCoordinates(room_x, room_y)) {
		args.push_back(CLValue(room_x)); // .. if so, add room coordinates to arguments
		args.push_back(CLValue(room_y));
	} else {
		args.push_back(CLValue(-1));
		args.push_back(CLValue(-1));
	}
	args.push_back(CLValue(btn));
