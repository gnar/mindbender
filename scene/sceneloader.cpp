...
void SceneLoader::ParseItemProperty(const std::string id, Item *item)
{
	if (id == "pos") {
		...
		CLValue key, val;
		key = CLValue(new CLString(context, "walkto_x")); val = CLValue(ParseInt()); item->set(key, val);
		key = CLValue(new CLString(context, "walkto_y")); val = CLValue(ParseInt()); item->set(key, val);
		key = CLValue(new CLString(context, "walkto_dir")); val = CLValue(ParseInt()); item->set(key, val);
	} else if (id == "hide") {
...
{
	BagItem *bagitem = new BagItem(context);
