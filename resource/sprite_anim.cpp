...
namespace Res
{
	...
	{
		SpriteState_Anim *as = (SpriteState_Anim*)state;
	...
	{
		// Load xml document
		std::auto_ptr<TiXmlDocument> doc(
			LoadXMLDocument(Manager.OpenFile(GetID()))
		);

		TiXmlElement *sprite_elem = doc->FirstChildElement("sprite");
		TiXmlElement *track_elem  = sprite_elem ? sprite_elem->FirstChildElement("track") : 0;
		while (track_elem)
		{
	...
	void Sprite_Anim::UpdateState(SpriteState_Anim &state, float time_passed)
	{
		...
		while (state.time >= 0.0f)
		{
			...
			if (state.track_pos >= (int)track.items.size()) // overrun?
			{
				...
				{
					case Track::REWIND: state.track_pos = 0; break;
					case Track::HALT: 
			...
			if (state.track_pos < 0) // underrun?
			{
				switch (track.loop_mode)
				{
					case Track::REWIND: assert(0);
					case Track::HALT: assert(0);
	...
	SpriteState_Anim::~SpriteState_Anim()
	{
	}
	...
	void SpriteState_Anim::Load(CLSerializer &S)
	{
		...
		S.IO(tmp);
		forward = tmp ? true : false; // bool: forward or backward playing

		S.IO(tmp);
		finished = tmp ? true : false; // bool: is animation finished?
	}
